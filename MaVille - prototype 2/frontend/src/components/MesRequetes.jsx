...
function MesRequetes() {
	...
	useEffect(() => {
		const fetchMesRequetes = async () => {
			try {
				const response = await axios.get("http://localhost:8080/api/users/requetes", {
	...
	const accepterSoumission = async (id) => {
		try {
			await axios.post(`http://localhost:8080/api/requetes/${id}/accept`, {}, { params: { email: auth.user.email } });
	...
	const fermerRequete = async (id) => {
		try {
			await axios.delete(`http://localhost:8080/api/requetes/${id}`);
