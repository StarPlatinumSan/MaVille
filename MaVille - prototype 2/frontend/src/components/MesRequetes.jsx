...
function MesRequetes() {
	...
	useEffect(() => {
		const fetchMesRequetes = async () => {
			try {
				const response = await axios.get("${process.env.REACT_APP_API_URL}/api/users/requetes", {
	...
	const accepterSoumission = async (id) => {
		try {
			await axios.post(`${process.env.REACT_APP_API_URL}/api/requetes/${id}/accept`, {}, { params: { email: auth.user.email } });
	...
	const fermerRequete = async (id) => {
		try {
			await axios.delete(`${process.env.REACT_APP_API_URL}/api/requetes/${id}`);
